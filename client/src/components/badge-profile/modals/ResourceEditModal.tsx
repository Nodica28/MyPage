...
interface ResourceEditModalProps {
  ...
  sectionId: string | number | undefined; // Updated to match ProfilePreview state
  isSaving: boolean;
  onClose: () => void;
...
export function ResourceEditModal({
  ...
  React.useEffect(() => {
    if (resource && open) {
      setEditingTitle(resource.title || resource.name || "");
      setEditingDescription(resource.description || "");
      setEditingType(resource.type);
      setEditingUrl(resource.url);

      // Handle thumbnail
      if (resource.thumbnail?.startsWith("#")) {
        setEditingThumbnailType("color");
        setEditingThumbnailColor(resource.thumbnail);
        setEditingThumbnailUrl("");
      } else if (resource.thumbnail) {
        setEditingThumbnailType("image");
        setEditingThumbnailUrl(resource.thumbnail);
  ...
  const handleSave = async () => {
    ...
    const updatedResource: Resource = {
      ...
      size: resource.size // Preserve existing size
    };
    ...
    >
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-4">
          <div>
            <Label htmlFor="globalEditingTitle">Title</Label>
            <Input
              id="globalEditingTitle"
              value={editingTitle}
          ...
          <div>
            <Label htmlFor="globalEditingDescription">
              Description (optional)
            </Label>
            <Textarea
              id="globalEditingDescription"
          ...
          <div>
            <Label htmlFor="globalEditingType">Type</Label>
            <Select
              value={editingType}
              onValueChange={(value) =>
              ...
              disabled={isSaving}
            >
              <SelectTrigger id="globalEditingType">
          ...
          <div>
            <Label htmlFor="globalEditingUrl">
